...
# Коэффициенты CO2
K_ELEC = 0.4   # кг на кВт·ч
K_CAR  = 0.15  # кг на км
K_WASTE = 0.5  # кг на кг мусора
...
def start(msg):
    ...
    bot.send_message(msg.chat.id,
        ...
        "(Просто отправьте число или фото счётчика — я пока принимаю только числа, фото в разработке)"
    )
    bot.register_next_step_handler(msg, step_elec)

def step_elec(msg):
    if msg.content_type == 'photo':
        bot.send_message(msg.chat.id, "📸 Спасибо за фото! Но я пока не умею распознавать показания. Введите, пожалуйста, число:")
        bot.register_next_step_handler(msg, step_elec)
        return
    try:
...
def step_waste(msg):
    ...
    resp = (
        ...
        "Советы по снижению: https://www.un.org/ru/actnow/ten-actions"
    )
